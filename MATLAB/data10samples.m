...
DataMat = zeros(15,4); %Predefine Data Matrix
AllAveAmb = zeros(1,n);
AllStdAmb = zeros(1,n);
Std2Qui = zeros(1,n);
Std2Med = zeros(1,n);
...
for c = 1:n
        ...
        DataMat(:,1,c) = xlsread(filearray(c,1), 'A2:A16'); %Ambient 1st col
        DataMat(:,2,c) = xlsread(filearray(c,1), 'B2:B16'); %Quiet is 2nd col
        DataMat(:,3,c) = xlsread(filearray(c,1), 'C2:C16'); %Medium is 3rd col
        DataMat(:,4,c) = xlsread(filearray(c,1), 'D2:D16'); %Loud is 4th col
        ...
        hold on
        scatter(1:15, DataMat(:,1,c), sz, 'r') %Ambient
        scatter(1:15, DataMat(:,2,c), sz, 'g') %Quiet
        scatter(1:15, DataMat(:,3,c), sz, 'b') %Medium
        scatter(1:15, DataMat(:,4,c), sz, 'k') %Loud
        ylim([10 110])
        title(DataLoc)
        xlabel('Trials')
